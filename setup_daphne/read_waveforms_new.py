    ...
    while keep_plotting:
        ...
        for g,AFE in enumerate(tqdm(AFEs, desc='Processing AFEs: ')):
            ...
            for d,channel in enumerate(tqdm(channels, desc='Processing channels in AFE: ')):
                ...
                # find baseline
                baseline = np.mean(rec[d][0:50])
                corrected_waveform = np.array(rec[d][0:-1]) - baseline
                
                # calculate signal amplitude and overshot amplitude
                signal_amplitude = - np.min(corrected_waveform)
                overshot_amplitude = np.max(corrected_waveform)
                
                #print(f"Channel {channel}, Baseline: {baseline:.2f}, min_value: {}")
                
                #plot.plot(rec[d][0:-1]-np.mean(rec[d][0:-1][0:50]), linewidth=0.5,color=colors[channel], label=f'ch {channel}')
                plot.plot(corrected_waveform, linewidth=0.5, color=colors[channel],
                label=f'ch {channel}\nSignal: {signal_amplitude:.2f}, Overshoot: {overshot_amplitude:.2f}')
                plot.set_xlabel('samples', fontsize=8)
                plot.legend(loc='lower right',fontsize='small',framealpha=0.5)
