...
parser = argparse.ArgumentParser(description="DAPHNE LED Multi-Day Normalized Reference Analysis")
parser.add_argument(
    "--runs",
    type=str,
    required=True,
...
for (afe, ch, peak_idx), trend in history_data.items():
    ...
    # ⭐ 修改 1：计算所有天数的幅值平均值作为 Reference Base
    ref_amp = np.mean(amps)
    
    if ref_amp == 0:
        continue
        
    # ⭐ 修改 2：计算均值的误差传递 (Standard Error of the Mean for independent measurements)
    ref_err = np.sqrt(np.sum(np.square(errs))) / len(errs)

    normalized_amps = [amp / ref_amp for amp in amps]
    ...
    for amp, err in zip(amps, errs):
        if amp == 0:
            ...
            normalized_errs.append((amp / ref_amp) * rel_err)

    # ⭐ 修改 3：将所有天数（包含第一天）的数据归类到对应的日期中
    for d, ratio in zip(dates, normalized_amps):
        date_label = d.strftime("%Y-%m-%d")
        if date_label not in ratios_by_date:
            ratios_by_date[date_label] = []
        ratios_by_date[date_label].append(ratio)
        
    led_labels.append(f"AFE{afe}_CH{ch}_P{peak_idx}")

    # --- Plot 1: Normalized Scanner Stability (Run / Mean Reference over Time) ---
    ax1.errorbar(
        dates,
        normalized_amps,
...
# --- Plot 1 Formatting ---
ax1.set_title("Normalized Scanner Stability (Run / All-Day Mean)", fontsize=12, fontweight="bold")
ax1.set_xlabel("Time", fontsize=11)
ax1.set_ylabel("Normalized Amplitude (Run / All-Day Mean)", fontsize=11)
ax1.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))
ax1.xaxis.set_major_locator(mdates.DayLocator(interval=1))
ax1.grid(True, linestyle="--", alpha=0.5)
ax1.axhline(1.0, color="black", linestyle="--", alpha=0.7, label="Mean Base (1.0)")

# --- Plot 2 Formatting (分色多组直方图绘制) ---
all_ratios = [r for r_list in ratios_by_date.values() for r in r_list]
if all_ratios:
    ratio_bins = np.linspace(min(min(all_ratios), 0.9), max(max(all_ratios), 1.1), 20)
else:
    ratio_bins = np.linspace(0.9, 1.1, 10)

# 现成标准色
colors_palette = ["#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", "#e377c2"]

# 提取所有有数据的日期并排序（此时包含了第一天）
...
ax2.set_title("Distribution of All Run / All-Day Mean Ratios", fontsize=12, fontweight="bold")
ax2.set_xlabel("Normalized Value (Run / All-Day Mean)", fontsize=11)
...
# ==========================================================
print("\n" + "="*60)
print(f"📊 DAPHNE Multi-Day Normalized Screening Report (Total: {len(led_labels)} Channels)")
print("="*60)
for d in sorted_dates:
    day_data = ratios_by_date[d]
    print(f"📅 Date: {d} | Samples: {len(day_data):<3} | Mean Ratio: {np.mean(day_data):.3f} | Std: {np.std(day_data):.3f}")
print("="*60 + "\n")
