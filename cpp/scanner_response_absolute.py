...
def main():
    ...
    print(f"🔄 正在计算 Run {args.run} 相对 Run {args.ref} 的对称百分比偏差...")
    ...
    for row_idx, afe in enumerate(args.afe_list):
        for ch_idx, ch in enumerate(args.ch_list):
            ...
            if (amps_test is not None) and (amps_ref is not None):
                ...
                # ⭐ 仅在此处修改为新公式: (run - ref) / ((run + ref) / 2)
                denominator = (amps_test + amps_ref) / 2.0
                deviation_pct = np.where(denominator != 0, ((amps_test - amps_ref) / denominator) * 100.0, 0.0)
    ...
    cbar.set_label('Absoulted Deviation (%)', fontsize=12, labelpad=10)
    ...
    # 全局大标题
    fig.suptitle(f"Module Response 4 (Absolute Comparison)\n \n Run {args.run} vs Reference Run {args.ref}",
                 fontsize=14)

    print("?? 对称均值新公式融合完毕，画图细节已全部保留。")
    plt.show()
